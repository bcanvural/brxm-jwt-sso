...
<#-- @ftlvariable name="document" type="com.bloomreach.ps.beans.Banner" -->
<#if document??>
    <div>
        <#if hst.isNodeType(document.node, 'brxmjwtsso:bannerdocument')>
            <a href="<@hst.link hippobean=document.link />">
                <figure style="position: relative">
                    ...
                    <figcaption
                            style="position:absolute; top:20px; left:20px; z-index:100; color:white; background: rgba(51, 122, 183, 0.7); width:60%; padding:0 20px 20px 20px; text-shadow: 0 1px 2px rgba(0, 0, 0, .6);">
                        <#if document.title??>
        ...
        <#elseif editMode>
            <figure style="position: relative">
                ...
                "${document.node.path}" is not of the correct type, please select or create a Banner document.
            </figure>
        </#if>
    </div>
<#elseif editMode>
